...
## Types
  ...
  - [1.1](#types--primitives) **Primitives**: When you access a primitive type you work directly on its value.
    ...
    - Symbols and BigInts cannot be faithfully polyfilled, so they should not be used when targeting browsers/environments that donÅft support them natively.
...
## References
  ...
  - [2.1](#references--prefer-const) Use `const` for all of your references; avoid using `var`. eslint: [`prefer-const`](https://eslint.org/docs/rules/prefer-const.html), [`no-const-assign`](https://eslint.org/docs/rules/no-const-assign.html)
    ...
    > Why? This ensures that you canÅft reassign your references, which can lead to bugs and difficult to comprehend code.
...
## Objects
  ...
  - [3.5](#objects--grouped-shorthand) Group your shorthand properties at the beginning of your object declaration.
    ...
    > Why? ItÅfs easier to tell which properties are using the shorthand.
...
## Arrays
  ...
  - [4.7](#arrays--callback-return) Use return statements in array method callbacks. ItÅfs ok to omit the return if the function body consists of a single statement returning an expression without side effects, following [8.2](#arrows--implicit-return). eslint: [`array-callback-return`](https://eslint.org/docs/rules/array-callback-return)
...
## Functions
  ...
  - [7.1](#functions--declarations) Use named function expressions instead of function declarations. eslint: [`func-style`](https://eslint.org/docs/rules/func-style)
    ...
    > Why? Function declarations are hoisted, which means that itÅfs easy - too easy - to reference the function before it is defined in the file. This harms readability and maintainability. If you find that a functionÅfs definition is large or complex enough that it is interfering with understanding the rest of the file, then perhaps itÅfs time to extract it to its own module! DonÅft forget to explicitly name the expression, regardless of whether or not the name is inferred from the containing variable (which is often the case in modern browsers or when using compilers such as Babel). This eliminates any assumptions made about the ErrorÅfs call stack.
  ...
  - [7.7](#es6-default-parameters) Use default parameter syntax rather than mutating function arguments.
    ...
    function handleThings(opts) {
      // No! We shouldnÅft mutate function arguments.
  ...
  - [7.11](#functions--signature-spacing) Spacing in a function signature. eslint: [`space-before-function-paren`](https://eslint.org/docs/rules/space-before-function-paren) [`space-before-blocks`](https://eslint.org/docs/rules/space-before-blocks)
    ...
    > Why? Consistency is good, and you shouldnÅft have to add or remove a space when adding or removing a name.
  ...
  - [7.14](#functions--spread-vs-apply) Prefer the use of the spread operator `...` to call variadic functions. eslint: [`prefer-spread`](https://eslint.org/docs/rules/prefer-spread)
    ...
    > Why? ItÅfs cleaner, you donÅft need to supply a context, and you can not easily compose `new` with `apply`.
...
## Arrow Functions
  ...
  - [8.4](#arrows--one-arg-parens) Always include parentheses around arguments for clarity and consistency. eslint: [`arrow-parens`](https://eslint.org/docs/rules/arrow-parens.html)
    ...
    // bad
    [1, 2, 3].map(number => (
      `A long string with the ${number}. ItÅfs so long that we donÅft want it to take up space on the .map line!`
    ));
...
## Classes & Constructors
  ...
  - [9.4](#constructors--tostring) ItÅfs okay to write a custom `toString()` method, just make sure it works successfully and causes no side effects.
...
## Modules
  ...
  - [10.1](#modules--use-them) Always use modules (`import`/`export`) over a non-standard module system. You can always transpile to your preferred module system.
    ...
    > Why? Modules are the future, letÅfs start using the future now.

    ```javascript
    // bad
    const SaltoStyleGuide = require('./SaltoStyleGuide');
    module.exports = SaltoStyleGuide.es6;

    // ok
    import SaltoStyleGuide from './SaltoStyleGuide';
    export default SaltoStyleGuide.es6;

    // best
    import { es6 } from './SaltoStyleGuide';
    export default es6;
    ```
  ...
  - [10.2](#modules--no-wildcard) Do not use wildcard imports.
    ...
    ```javascript
    // bad
    import * as SaltoStyleGuide from './SaltoStyleGuide';
  ...
  - [10.3](#modules--no-export-from-import) And do not export directly from an import.
    ...
    // bad
    // filename es6.js
    export { es6 as default } from './SaltoStyleGuide';
...
## Iterators and Generators
  ...
  - [11.1](#iterators--nope) DonÅft use iterators. Prefer JavaScriptÅfs higher-order functions instead of loops like `for-in` or `for-of`. eslint: [`no-iterator`](https://eslint.org/docs/rules/no-iterator.html) [`no-restricted-syntax`](https://eslint.org/docs/rules/no-restricted-syntax)
  ...
  - [11.2](#generators--nope) DonÅft use generators for now.

    > Why? They donÅft transpile well to ES5.
...
## Variables
  ...
  - [13.2](#variables--one-const) Use one `const` or `let` declaration per variable or assignment. eslint: [`one-var`](https://eslint.org/docs/rules/one-var.html)
    ...
    > Why? ItÅfs easier to add new variable declarations this way, and you never have to worry about swapping out a `;` for a `,` or introducing punctuation-only diffs. You can also step through each declaration with the debugger, instead of jumping through all of them at once.
  ...
  - [13.5](#variables--no-chain-assignment) DonÅft chain variable assignments. eslint: [`no-multi-assign`](https://eslint.org/docs/rules/no-multi-assign)
...
## Hoisting
  ...
  - [14.1](#hoisting--about) `var` declarations get hoisted to the top of their closest enclosing function scope, their assignment does not. `const` and `let` declarations are blessed with a new concept called [Temporal Dead Zones (TDZ)](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Statements/let#Temporal_dead_zone). ItÅfs important to know why [typeof is no longer safe](http://es-discourse.com/t/why-typeof-is-no-longer-safe/15).

    ```javascript
    // we know this wouldnÅft work (assuming there
...
## Comparison Operators & Equality
  ...
  eslint: [`no-mixed-operators`](https://eslint.org/docs/rules/no-mixed-operators.html)
    ...
    > Why? This improves readability and clarifies the developerÅfs intention.
...
## Blocks
  ...
  - [16.2](#blocks--cuddled-elses) If youÅfre using multiline blocks with `if` and `else`, put `else` on the same line as your `if` blockÅfs closing brace. eslint: [`brace-style`](https://eslint.org/docs/rules/brace-style.html)
...
## Comments
  ...
  - [18.2](#comments--singleline) Use `//` for single line comments. Place single line comments on a newline above the subject of the comment. Put an empty line before the comment unless itÅfs on the first line of a block.
  ...
  - [18.4](#comments--actionitems) Prefixing your comments with `FIXME` or `TODO` helps other developers quickly understand if youÅfre pointing out a problem that needs to be revisited, or if youÅfre suggesting a solution to the problem that needs to be implemented. These are different than regular comments because they are actionable. The actions are `FIXME: -- need to figure this out` or `TODO: -- need to implement`.
  ...
  - [18.5](#comments--fixme) Use `// FIXME:` to annotate problems.
    ...
    class Calculator extends Abacus {
      constructor() {
        ...
        // FIXME: shouldnÅft use a global here
        total = 0;
      }
    }
    ```
...
## Whitespace
  ...
  - [19.5](#whitespace--newline-at-end) End files with a single newline character. eslint: [`eol-last`](https://github.com/eslint/eslint/blob/master/docs/rules/eol-last